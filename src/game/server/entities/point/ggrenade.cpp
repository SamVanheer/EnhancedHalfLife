...
void CGrenade::Explode(TraceResult* pTrace, int bitsDamageType)
{
	...
	RadiusDamage(this, oldOwner, pev->dmg, CLASS_NONE, bitsDamageType);
...
{
	const Vector vecSpot = GetAbsOrigin() - pev->velocity.Normalize() * 32; // trace starts here!
	TraceResult tr;
...
CGrenade* CGrenade::ShootTimed(CBaseEntity* pOwner, Vector vecStart, Vector vecVelocity, float time)
{
	...
	{
		pGrenade->pev->nextthink = gpGlobals->time;
		pGrenade->pev->velocity = vec3_origin;
	}
