...
void CTalkMonster::RunTask(Task_t* pTask)
{
	...
	case TASK_TLK_LOOK_AT_CLIENT:
	{
		auto pPlayer = UTIL_FindClientInPVS(this);
...
void CTalkMonster::Touch(CBaseEntity* pOther)
{
	...
	if (pOther->IsPlayer())
	{
		...
		const float speed = fabs(pOther->GetAbsVelocity().x) + fabs(pOther->GetAbsVelocity().y);
		if (speed > 50)
		{
...
bool CTalkMonster::OkToSpeak()
{
	...
	if (!IsAlive() || IsNullEnt(UTIL_FindClientInPVS(this)))
		return false;
...
Schedule_t* CTalkMonster::GetScheduleOfType(int Type)
{
	...
	case SCHED_IDLE_STAND:
	{
		...
		{
			if (CBasePlayer* pPlayer = UTIL_FindClientInPVS(this); pPlayer)
			{
				// watch the client.
				UTIL_MakeVectors(pPlayer->GetAbsAngles());
				if ((pPlayer->GetAbsOrigin() - GetAbsOrigin()).Length2D() < TLK_STARE_DIST &&
