...
void COsprey::DeployThink()
{
	...
	TraceResult tr;
	UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + Vector(0, 0, -WORLD_BOUNDARY), IgnoreMonsters::Yes, this, &tr);
	CSoundEnt::InsertSound(bits_SOUND_DANGER, tr.vecEndPos, 400, 0.3);

	Vector vecSrc = GetAbsOrigin() + vecForward * 32 + vecRight * 100 + vecUp * -96;
...
CBaseMonster* COsprey::MakeGrunt(const Vector& vecSrc)
{
	...
	for (int i = 0; i < m_iUnits; i++)
	{
		if (m_hGrunt[i] == nullptr || !m_hGrunt[i]->IsAlive())
		{
			...
			CBaseEntity* pEntity = Create("monster_human_grunt", vecSrc, GetAbsAngles());
...
void COsprey::Flight()
{
	...
	const float m_flIdealtilt = (160 - flSpeed) / 10.0f;
...
void COsprey::ShowDamage()
{
	...
	{
		const Vector vecSrc = GetAbsOrigin() + gpGlobals->v_right * -340;
