...
bool CHAssassin::CheckMeleeAttack1(float flDot, float flDist)
{
	if (m_flNextJump < gpGlobals->time && (flDist <= 128 || HasMemory(bits_MEMORY_BADJUMP)) && m_hEnemy != nullptr)
	{
		...
		const Vector vecDest = GetAbsOrigin() + Vector(RANDOM_FLOAT(-64, 64), RANDOM_FLOAT(-64, 64), 160);

		UTIL_TraceHull(GetAbsOrigin() + Vector(0, 0, 36), vecDest + Vector(0, 0, 36), IgnoreMonsters::No, Hull::Human, this, &tr);
...
bool CHAssassin::CheckRangeAttack2(float flDot, float flDist)
{
	...
	{
		const Vector vecToss = CheckThrow(this, GetGunPosition(), m_hEnemy->Center(), flDist, 0.5); // use dist as speed to get there in 1 second
...
void CHAssassin::RunAI()
{
	...
	if (GetRenderAmount() > m_iTargetRanderamt)
	{
		...
		SetRenderAmount(std::max(GetRenderAmount() - 50, static_cast<float>(m_iTargetRanderamt)));
		SetRenderMode(RenderMode::TransTexture);
	}
	else if (GetRenderAmount() < m_iTargetRanderamt)
	{
		SetRenderAmount(std::min(GetRenderAmount() + 50, static_cast<float>(m_iTargetRanderamt)));
		if (GetRenderAmount() == 255)
