			...
			if (m_iIsATeam[row] != TeamType::No)
			{
				...
				case COLUMN_NAME:
					...
					if (team_info)
					{
						...
						{
							snprintf(sz2, sizeof(sz2), "(%d %s)", team_info->players, CHudTextMessage::BufferedLocaliseTextString("#Player_plural"));
						}
			...
			else
			{
				...
				case COLUMN_NAME:
					...
					if (g_pTrackerUser)
					{
						...
						{
							snprintf(sz, sizeof(sz), "   (%s)", trackerName);
							pLabel->setText2(sz);
						}
					}
					*/
...
void ScorePanel::mousePressed(MouseCode code, Panel* panel)
{
	...
	else if (m_iHighlightRow >= 0)
	{
		...
		if (iPlayer > 0)
		{
			...
			{
				const std::size_t MaxTextLength = 1024;
				const std::size_t MaxLocalizedTextLength = 256;

				char string[MaxTextLength];

				if (GetClientVoiceMgr()->IsPlayerBlocked(iPlayer))
				{
					char string1[MaxLocalizedTextLength];
				...
				{
					char string1[MaxLocalizedTextLength];
					char string2[MaxLocalizedTextLength];
