...
/**
*	@brief Creates a new CMessageWindowPanel
*/
...
/**
*	@brief InputSignal handler for the main viewport
*/
...
void TeamFortressViewport::UpdateSpectatorPanel()
{
	...
	if (g_iUser1 && gHUD.m_pCvarDraw->value && !gHUD.m_iIntermission)	// don't draw in dev_overview mode
	{
		...
		if (!m_pSpectatorPanel->isVisible())
		{
			...
			snprintf(tempString, sizeof(tempString), "%c%s", HUD_PRINTCENTER, CHudTextMessage::BufferedLocaliseTextString("#Spec_Duck"));
		...
		snprintf ( szText, sizeof(szText), "%d:%02d\n", (timer / 60), (timer % 60) );
...
bool TeamFortressViewport::AllowedToPrintText()
{
	...
	if (m_pCurrentMenu && g_iPlayerClass == 0)
	{
		...
		if (iId == MENU_INTRO || iId == MENU_CLASSHELP)
			return false;
	}
...
void TeamFortressViewport::UpdateCursorState()
{
	...
	if (m_pSpectatorPanel->m_menuVisible || m_pCurrentMenu || GetClientVoiceMgr()->IsInSquelchMode())
	{
...
bool TeamFortressViewport::KeyInput(int down, int keynum, const char* pszCurrentBinding)
{
	// Open Text Window?
