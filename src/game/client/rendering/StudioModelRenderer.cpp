...
void CStudioModelRenderer::StudioSetUpTransform(int trivial_accept)
{
	...
	if (m_pCurrentEntity->curstate.movetype == Movetype::Step)
	{
		...
		modelpos = modelpos + (m_pCurrentEntity->origin - m_pCurrentEntity->latched.prevorigin) * f;
...
void CStudioModelRenderer::StudioSaveBones()
{
	...
	for (i = 0; i < m_pStudioHeader->numbones; i++)
	{
		safe_strcpy(m_nCachedBoneNames[i], pbones[i].name);
...
bool CStudioModelRenderer::StudioDrawPlayer(int flags, entity_state_t* pplayer)
{
	...
	if (pplayer->gaitsequence)
	{
		...
		const Vector orig_angles = m_pCurrentEntity->angles;
